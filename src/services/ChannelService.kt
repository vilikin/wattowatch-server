...
class PersistedChannel(
    ...
    url: String?,
    imageUrl: String?
) : Channel(sourceSystem, idInSourceSystem, name, url, imageUrl) {
    companion object {
        fun fromRow(row: Row, channelPrefix: String = ""): PersistedChannel {
            return PersistedChannel(
                ...
                row.stringOrNull("${channelPrefix}image_url")
            )
        }
    }
}
...
class ChannelService(private val hikariDataSource: HikariDataSource) {
    fun createChannel(channel: Channel): PersistedChannel {
        return using(sessionOf(hikariDataSource)) { session ->
            session.run(
                queryOf(
                    """
                        INSERT INTO channels (source_system, id_in_source_system, name, url, image_url)
                        VALUES (:source_system, :id_in_source_system, :name, :url, :image_url)
                        RETURNING *
                    """,
                    mapOf(
    ...
    fun removeSubscription(channel: PersistedChannel, user: PersistedUser) {
        return using(sessionOf(hikariDataSource)) { session ->
            session.run(
                queryOf(
                    "DELETE FROM user_is_subscribed_to_channel WHERE user_id = :user_id AND channel_id = :channel_id",
                    mapOf(
                        "user_id" to user.id,
                        "channel_id" to channel.id
                    )
    ...
    fun getChannelsUserIsSubscribedTo(userId: Int): List<PersistedChannel> {
        return using(sessionOf(hikariDataSource)) { session ->
            session.run(
                queryOf(
                    """
                        SELECT id, source_system, id_in_source_system, name, url, image_url FROM user_is_subscribed_to_channel
